...
class TeamList implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
        $this->base = 'mkd_team_list';

        add_action('vc_before_init', array($this, 'vcMap'));

	    //Team category filter
	    add_filter( 'vc_autocomplete_mkd_team_list_category_callback', array( &$this, 'teamCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Team category render
	    add_filter( 'vc_autocomplete_mkd_team_list_category_render', array( &$this, 'teamCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Team selected projects filter
	    add_filter( 'vc_autocomplete_mkd_team_list_selected_projects_callback', array( &$this, 'teamIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Team selected projects render
	    add_filter( 'vc_autocomplete_mkd_team_list_selected_projects_render', array( &$this, 'teamIdAutocompleteRender', ), 10, 1 ); // Render exact team. Must return an array (label,value)
    }
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map( array(
				    'name'                      => esc_html__('Mikado Team List', 'mkd-core'),
				    'base'                      => $this->getBase(),
				    ...
				    'params'                    => array(
					    ...
					    array(
						    ...
						    'heading'     => esc_html__( 'Number of team members per page', 'mkd-core' ),
						    'description' => esc_html__( 'Set number of items for your team list. Enter -1 to show all.', 'mkd-core' ),
						    'value'       => '-1'
					    ),
					    array(
						    'type'        => 'autocomplete',
						    'param_name'  => 'category',
						    'heading'     => esc_html__( 'One-Category Team List', 'mkd-core' ),
                        ...
                        array(
                            ...
                            'heading'     => esc_html__('Display Description', 'mkd-core'),
                        ...
                        array(
                            ...
                            'heading'     => esc_html__('Display Social Icons', 'mkd-core'),
                            'value'       => array_flip(depot_mikado_get_yes_no_select_array(false)),
                            'dependency' => array('element' => 'team_member_layout', 'value' => 'info-bellow')
                        ),
                        array(
                            ...
                            'heading'    => esc_html__('Team Member Image Layout', 'mkd-core'),
                            'value'      => array(
					    ...
					    array(
						    ...
						    'heading'     => esc_html__( 'Team Member Layout', 'mkd-core' ),
						    'value'       => array(
								esc_html__( 'Info Bellow', 'mkd-core' ) => 'info-bellow',
								esc_html__( 'Info on Hover', 'mkd-core' ) => 'info-hover',
						    ),
						    'group'       => esc_html__( 'Content Layout', 'mkd-core' )
					    )
				    )
    ...
    public function render($atts, $content = null) {
	    ...
	    $html = mkd_core_get_cpt_shortcode_module_template_part('team', 'team-holder', '', $params, $additional_params);
	...
	public function teamCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['team_category_title'] ) > 0 ) ? esc_html__( 'Category', 'mkd-core' ) . ': ' . $value['team_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $team_category ) ) {
				...
				if ( ! empty( $team_category_title ) ) {
					$team_category_title_display = esc_html__( 'Category', 'mkd-core' ) . ': ' . $team_category_title;
				}
	...
	public function teamIdAutocompleteRender( $query ) {
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $team ) ) {
				...
				$team_title_display = '';
				if ( ! empty( $team_title ) ) {
					$team_title_display = ' - ' . esc_html__( 'Title', 'mkd-core' ) . ': ' . $team_title;
				}

				$team_id_display = esc_html__( 'Id', 'mkd-core' ) . ': ' . $team_id;
