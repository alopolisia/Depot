...
class depotMikadoImport {
	...
	function init_mkd_import() {
		if(isset($_REQUEST['import_option'])) {
			...
			if($import_option == 'content'){
				$this->import_content('depot_content.xml');
	...
	public function import_options($file){
		$options = $this->file_options($file);
		update_option( 'mkd_options_depot', $options);
	...
	function mkd_admin_import() {
		if (mkd_core_theme_installed()) {
			...
			$this->pagehook = add_submenu_page(
				...
				array($depot_Framework->getSkin(), 'renderImport')
			);

			add_action('admin_print_scripts-'.$this->pagehook, 'depot_mikado_enqueue_admin_scripts');
			add_action('admin_print_styles-'.$this->pagehook, 'depot_mikado_enqueue_admin_styles');
...
function mkd_init_import_object(){
	global $depot_import_object;
	$depot_import_object = new depotMikadoImport();
}

add_action('init', 'mkd_init_import_object');

if(!function_exists('depot_mikado_dataImport')){
	function depot_mikado_dataImport(){
		global $depot_import_object;

		if ($_POST['import_attachments'] == 1)
			$depot_import_object->attachments = true;
		else
			$depot_import_object->attachments = false;

		$folder = "depot/";
		if (!empty($_POST['example']))
			$folder = $_POST['example']."/";

		$depot_import_object->import_content($folder.$_POST['xml']);

		die();
	}

	add_action('wp_ajax_mkd_dataImport', 'depot_mikado_dataImport');
}

if(!function_exists('depot_mikado_widgetsImport')){
	function depot_mikado_widgetsImport(){
		global $depot_import_object;
		...
		$depot_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

		die();
	}

	add_action('wp_ajax_mkd_widgetsImport', 'depot_mikado_widgetsImport');
}

if(!function_exists('depot_mikado_optionsImport')){
	function depot_mikado_optionsImport(){
		global $depot_import_object;
		...
		$depot_import_object->import_options($folder.'options.txt');

		die();
	}

	add_action('wp_ajax_mkd_optionsImport', 'depot_mikado_optionsImport');
}

if(!function_exists('depot_mikado_otherImport')){
	function depot_mikado_otherImport(){
		global $depot_import_object;
		...
		$depot_import_object->import_options($folder.'options.txt');
		$depot_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
		$depot_import_object->import_menus($folder.'menus.txt');
		$depot_import_object->import_settings_pages($folder.'settingpages.txt');

		if ( mkd_core_is_revolution_slider_installed() ) {
			$depot_import_object->rev_slider_import( $folder );
		}

		die();
	}

	add_action('wp_ajax_mkd_otherImport', 'depot_mikado_otherImport');
}