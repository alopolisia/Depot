...
class ImageGallery implements Lib\ShortcodeInterface {
		...
		vc_map(array(
			...
			'params'                    => array(
				array(
					...
					'value'      => array(
						esc_html__('Image Grid', 'mkd-core')	=> 'image_grid',
				...
				array(
					...
					'heading'	 => esc_html__('Slide Animation', 'mkd-core'),
					'value'		 => array(
						esc_html__('Slide', 'mkd-core')	=> 'slide',
						esc_html__('Fade', 'mkd-core') => 'fade',
						esc_html__('Fade Up', 'mkd-core') => 'fadeUp',
						esc_html__('Back Slide', 'mkd-core') => 'backSlide',
						esc_html__('Go Down', 'mkd-core') => 'goDown'
					),
				...
				array(
					...
					'heading'	  => esc_html__('Open PrettyPhoto On Click', 'mkd-core'),
				...
				array(
					...
					'heading'     => esc_html__('Grayscale Images', 'mkd-core'),
					'value'       => array_flip(depot_mikado_get_yes_no_select_array(false)),
					'dependency'  => array('element' => 'type', 'value' => array('image_grid'))
				),
				array(
	...
	public function render($atts, $content = null) {
		...
		if ($params['navigation'] === 'yes' && $params['pagination'] === 'yes' ) {
			$params['slider_classes'] = 'mkd-nav-pag-enabled';
		}
		...
		$html = mkd_core_get_shortcode_module_template_part('templates/' . $template, 'image-gallery', '', $params);
	...
	private function getGalleryClasses($params) {
		...
		$holderClasses .= !empty($params['number_of_columns']) ? ' mkd-ig-columns-' . $params['number_of_columns'] : ' mkd-ig-columns-3';
		$holderClasses .= !empty($params['space_between_columns']) ? ' mkd-ig-' . $params['space_between_columns'] . '-space' : ' mkd-ig-normal-space';
		$holderClasses .= $params['grayscale'] === 'yes' ? ' mkd-ig-grayscale' : '';
