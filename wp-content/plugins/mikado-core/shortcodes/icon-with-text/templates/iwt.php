<div <?php depot_mikado_class_attribute($holder_classes); ?>>
	...
	<div class="mkd-iwt-content" <?php depot_mikado_inline_style($content_styles); ?>>
		<?php if(!empty($title)) { ?>
			<<?php echo esc_attr($title_tag); ?> class="mkd-iwt-title" <?php depot_mikado_inline_style($title_styles); ?>>
				<?php if(!empty($link)) : ?>
					<a itemprop="url" href="<?php echo esc_url($link); ?>" target="<?php echo esc_attr($target); ?>">
				<?php endif; ?>
		...
		<?php } ?>
		<?php if(!empty($text)) { ?>
			<p class="mkd-iwt-text" <?php depot_mikado_inline_style($text_styles); ?>><?php echo wp_kses_post($text); ?></p>
		<?php } ?>
	</div>
</div>