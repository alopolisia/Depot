...
class MikadoInstagramWidget extends WP_Widget {
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				'name'    => 'image_size',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Image Size', 'mkd-instagram-feed' ),
				'options' => array(
					'thumbnail'           => esc_html__( 'Small', 'mkd-instagram-feed' ),
					'low_resolution'      => esc_html__( 'Medium', 'mkd-instagram-feed' ),
					'standard_resolution' => esc_html__( 'Large', 'mkd-instagram-feed' )
				)
			),
			array(
	...
	public function widget( $args, $instance ) {
		...
		if ( is_array( $images_array ) && count( $images_array ) ) { ?>
			<ul class="mkd-instagram-feed clearfix mkd-col-<?php echo esc_attr( $number_of_cols ); ?> <?php echo esc_attr( $widget_class ); ?>" <?php echo depot_mikado_get_inline_attrs( $slider_data ); ?>>
				<?php
				foreach ( $images_array as $image ) { ?>
					<li>
						<a href="<?php echo esc_url( $instagram_api->getHelper()->getImageLink( $image ) ); ?>" target="_blank">
							<?php echo depot_mikado_kses_img( $instagram_api->getHelper()->getImageHTML( $image, $image_size ) ); ?>
                            <span class="overlay-icon social-instagram"><i class="mkd-social-icon-widget fa fa-instagram"></i></span>
						</a>
					</li>
				<?php } ?>
			</ul>
